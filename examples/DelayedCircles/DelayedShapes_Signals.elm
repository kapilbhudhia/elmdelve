...
main =
    let
        ...
        drawingPositions =
            createDrawPositions <~ Window.dimensions ~ (delayedMousePositions delays)

        drawingPositionsWithRadius =
            attachRadius <~ (Signal.constant radiis) ~ drawingPositions
    in
...
displayCircles ( w, h ) circlesData =
    let
        data =
            List.map2 (,) (List.range 1 (List.length circlesData)) (List.reverse circlesData)
    in
